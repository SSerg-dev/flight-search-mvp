  ...
  if (flights.length === 0) {
    return `
      <section class="mx-auto mt-6 max-w-5xl rounded border border-dashed border-slate-300 bg-white p-6 text-center dark:border-slate-700 dark:bg-slate-900 sm:p-8" aria-live="polite">
        ${title ? `<p class="mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">${escapeHtml(title)}</p>` : ''}
        <h2 class="text-lg font-semibold text-slate-950 dark:text-slate-100">No matching flights found</h2>
        <p class="mt-2 text-sm text-slate-600 dark:text-slate-400">Try changing the date range or layover hours.</p>
      </section>
    `;
  }
  ...
  return `
    <section class="mx-auto mt-6 grid max-w-5xl gap-4 px-4 sm:px-0" aria-live="polite">
      <div class="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
        <div>
          <h2 class="text-xl font-semibold text-slate-950 dark:text-slate-100">${escapeHtml(title ?? `${flights.length} matching ${flights.length === 1 ? 'flight' : 'flights'}`)}</h2>
          <p class="mt-1 text-sm text-slate-600 dark:text-slate-300">Prices are estimates for the selected route.</p>
          ${createDateRangeSummary(query, dateRangeLabel)}
        </div>
        <label class="grid gap-1 text-sm font-medium text-slate-700 dark:text-slate-300" for="${escapeHtml(sortControlId)}">
          Sort results
          <select
...
function createDateRangeSummary(query, label) {
  ...
  return `<p class="mt-1 text-sm font-medium text-slate-700 dark:text-slate-300">${escapeHtml(label)} from ${escapeHtml(start)} to ${escapeHtml(end)}</p>`;
}
