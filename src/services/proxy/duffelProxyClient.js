...
export function buildDuffelProxyRequest(query) {
  return {
    provider: 'duffel',
    route: {
...
export async function fetchDuffelFlightOffers(query, { proxyUrl, fetchImpl = getDefaultFetch() } = {}) {
  if (!hasText(proxyUrl)) {
    throw new Error('Flight API proxy URL is required for Duffel mode.');
  }
  ...
  const request = buildDuffelProxyRequest(query);
  let response;
